...
δ(τ)=[abs(i)==minimum(abs.(τ)) ? 1 : 0 for i in τ]
c = 3e8
```
## Introducción
...
\end{aligned}
$$ {#eq-ai}
En donde $x_1(t)$ y $x_2(t)$ son la distancia de cada camino, y $\tau_2$ contempla el salto de fase provocado por la reflexión
$$
...
```{julia}
#| output: false
h(τ,t) = ai[1](t).*δ(τ.-τi[1](t)).+
         ai[2](t).*δ(τ.-τi[2](t))
```
Así como en el dominio de la frecuencia, @eq-H
```{julia}
#| output: false
H(f,t) = ai[1](t)*exp(-im*2π*f*τi[1](t))+ 
         ai[2](t)*exp(-im*2π*f*τi[2](t))
```
...
#| output: false
function Δθ(f, t)
  Δθ = 2π*f*(τi[2](t)-τi[1](t))+π
  return mod(Δθ+π, 2π)-π
end
```
...
p1 = plot(τ.*1e6, h(τ, t1) .|> abs, yformatter=(_->""), label="r=$(ceil(xi[1](t1))|>Int)m", ylims=(0, 0.006), xlims=(0, 8), xformatter=(_->""), title="h(τ, t)")
p3 = plot(τ.*1e6, h(τ, t2) .|> abs, yformatter=(_->""), label="r=$(ceil(xi[1](t2))|>Int)m", ylims=(0, 0.006), xlims=(0, 8), xformatter=(_->""))
p5 = plot(τ.*1e6, h(τ, t3) .|> abs, yformatter=(_->""), label="r=$(ceil(xi[1](t3))|>Int)m", ylims=(0, 0.006), xlims=(0, 8), xlabel="τ [ms]")
...
p1 = plot(τ.*1e6, ifft(H.(f, t1)) .|> abs |> fftshift, yformatter=(_->""), label="h(τ)", ylims=(0, 0.006), xlims=(0, 8), xformatter=(_->""), title="h(τ, t)")
p2 = plot(τ.*1e6, ifft(H.(f, t2)) .|> abs |> fftshift, yformatter=(_->""), label="h(τ)", ylims=(0, 0.006), xlims=(0, 8), xformatter=(_->""))
p3 = plot(τ.*1e6, ifft(H.(f, t3)) .|> abs |> fftshift, yformatter=(_->""), label="h(τ)", ylims=(0, 0.006), xlims=(0, 8), xlabel="τ [ms]")

xpos=0.005
params=(linecolor=:black, label="Ts", markershape=:vline, markercolor=:black, markersize=6)
plot!(p1, (τi[1](t1).+[0, Ts(t1)]).*1e6, [xpos, xpos]; params...)
plot!(p2, (τi[1](t2).+[0, Ts(t2)]).*1e6, [xpos, xpos]; params...)
plot!(p3, (τi[1](t3).+[0, Ts(t3)]).*1e6, [xpos, xpos]; params...)
params=(linestyle=:dot, linecolor=:black, label=false)
plot!(p1, (τi[1](t1).+[0, 0]).*1e6, [0, xpos]; params...)
plot!(p2, (τi[1](t2).+[0, 0]).*1e6, [0, xpos]; params...)
plot!(p3, (τi[1](t3).+[0, 0]).*1e6, [0, xpos]; params...)
plot!(p1, (τi[1](t1).+[Ts(t1), Ts(t1)]).*1e6, [0, xpos]; params...)
plot!(p2, (τi[1](t2).+[Ts(t2), Ts(t2)]).*1e6, [0, xpos]; params...)
plot!(p3, (τi[1](t3).+[Ts(t3), Ts(t3)]).*1e6, [0, xpos]; params...)
...
#| echo: false
#| label: fig-Wc
#| fig-cap: "Max $=a_1(r)+a_2(r)$, interferencia constructiva. Min $=a_1(r)-a_2(r)$, interferencia destructiva."
Hs = H.(f, t[end]) .|> abs
...
p2 = plot(f.*1e-9, Δθ.(f, t[end]), xlims=flims, xlabel="f [GHz]", link=:x, legend=false, ylabel="Δθ")
...
#| echo: false
#| label: fig-Xc
#| fig-cap: "Respuesta en frecuencia del canal y diferencia de fase a f_0 = 1 GHz. Se grafican además el valor máximo y mínimo de la respuesta en frecuencia, Max $=a_1(f_0)+a_2(f_0)$ (interferencia constructiva) y Min $=a_1(f_0)-a_2(f_0)$ (interferencia destructiva)."
t = t[(3*N/4 |> Int):N]
...
p2 = plot(xi[1].(t), Δθ.(1e9, t), xlims=rlims, xlabel="r [m]", link=:x, legend=:false, ylabel="Δθ")
